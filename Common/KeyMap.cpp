...
struct DefaultKeyMap {
	static KeyMapping defaultKeyboardMap()
	{
		...
		m[KeyDef(DEVICE_ID_KEYBOARD, NKCODE_W)] = CTRL_TRIANGLE;
		m[KeyDef(DEVICE_ID_KEYBOARD, NKCODE_D)] = CTRL_CIRCLE;
		m[KeyDef(DEVICE_ID_KEYBOARD, NKCODE_S)] = CTRL_CROSS;
		m[KeyDef(DEVICE_ID_KEYBOARD, NKCODE_Q)] = CTRL_LTRIGGER;
		m[KeyDef(DEVICE_ID_KEYBOARD, NKCODE_E)] = CTRL_RTRIGGER;
		m[KeyDef(DEVICE_ID_KEYBOARD, NKCODE_ENTER)] = CTRL_START;
#ifdef _WIN32
		m[KeyDef(DEVICE_ID_KEYBOARD, NKCODE_SPACE)] = CTRL_SELECT;
#else
