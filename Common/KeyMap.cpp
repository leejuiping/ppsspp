...
struct DefaultKeyMap {
	static KeyMapping defaultKeyboardMap()
	{
		...
		m[KeyDef(DEVICE_ID_KEYBOARD, KEYCODE_W)] = CTRL_TRIANGLE;
		m[KeyDef(DEVICE_ID_KEYBOARD, KEYCODE_D)] = CTRL_CIRCLE;
		m[KeyDef(DEVICE_ID_KEYBOARD, KEYCODE_S)] = CTRL_CROSS;
		m[KeyDef(DEVICE_ID_KEYBOARD, KEYCODE_Q)] = CTRL_LTRIGGER;
		m[KeyDef(DEVICE_ID_KEYBOARD, KEYCODE_E)] = CTRL_RTRIGGER;
		m[KeyDef(DEVICE_ID_KEYBOARD, KEYCODE_ENTER)] = CTRL_START;
#ifdef _WIN32
		m[KeyDef(DEVICE_ID_KEYBOARD, KEYCODE_SPACE)] = CTRL_SELECT;
#else
